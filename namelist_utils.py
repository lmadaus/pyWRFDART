...
def update_time_wrf(nmld, start, end):
    """
    Updates all relevant time variables in WRF namelist.input
    to reflect start and end (datetimes)
    """
    ndom = nmld['domains']['max_dom']
    nmld['time_control']['start_year'] = [start.year] * ndom
    nmld['time_control']['start_month'] = [start.month] * ndom
    nmld['time_control']['start_day'] = [start.day] * ndom
    nmld['time_control']['start_hour'] = [start.hour] * ndom
    nmld['time_control']['start_minute'] = [start.minute] * ndom
    nmld['time_control']['start_second'] = [start.second] * ndom
    nmld['time_control']['end_year'] = [end.year] * ndom
    nmld['time_control']['end_month'] = [end.month] * ndom
    nmld['time_control']['end_day'] = [end.day] * ndom
    nmld['time_control']['end_hour'] = [end.hour] * ndom
    nmld['time_control']['end_minute'] = [end.minute] * ndom
    nmld['time_control']['end_second'] = [end.second] * ndom
    # Also need the run time
    run_dt = end - start
    nmld['time_control']['run_days'] = run_dt.days
    hours, remainder = divmod(run_dt.seconds, 3600)
    minutes, seconds = divmod(remainder, 60)
    nmld['time_control']['run_hours'] = hours
    nmld['time_control']['run_minutes'] = minutes
    nmld['time_control']['run_seconds'] = seconds

    return nmld

def update_time_wps(nmld, start, end):
    """
    Updates all relevant time variables in WPS namelist.wps
    to reflect start and end (datetimes)
    """
    ndom = nmld['share']['max_dom']
    nmld['share']['start_date'] = [start.strftime('%Y-%m-%d_%H:%M:%S')] * ndom
    nmld['share']['end_date'] = [end.strftime('%Y-%m-%d_%H:%M:%S')] * ndom
    return nmld
...
def write_dart_namelist(nmld=None, mem=1, date=None, window_minutes=15, outname='input.nml'):
    ...
    if date is not None:
        ...
        nmld['obs_seq_coverage_nml']['last_analysis'] = [end_window.year, end_window.month, end_window.day,\
                                                   end_window.hour, end_window.minute, end_window.second]
        nmld['obs_diag_nml']['first_bin_center'] = [date.year, date.month, date.day,\
                                                   date.hour, date.minute, date.second]
        nmld['obs_diag_nml']['last_bin_center'] = [date.year, date.month, date.day,\
                                                   date.hour, date.minute, date.second]
